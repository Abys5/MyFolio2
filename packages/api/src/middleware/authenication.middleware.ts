...
export default (req: RequestWithUser, res: Express.Response, next: Express.NextFunction): void => {
    try {
        const token = req.headers['x-auth-token'] || req.cookies['x-auth-token'];

        if (token) {
            //const payload: any = jwt.verify(token, process.env.SECRET, {});
            jwt.verify(token, process.env.SECRET as jwt.Secret, {}, (err: jwt.VerifyErrors, payload: any) => {
                if (payload) {
                    Session.getSessionByToken(payload.token, sessionEntry => {
                        if (sessionEntry != null) {
                            User.getUserByID(sessionEntry.user_id, userEntry => {
                                if (userEntry != null) {
                                    req.user = userEntry;
                                    next();
                                } else {
